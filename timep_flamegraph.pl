...
# notice and the original CDDL license are included unchanged.
# ------------------------------------------------------------
#
...
foreach (<>) {
    ...
    if ($stackreverse) {
                ...
                my ($stack, $samples1, $samples2) = /^(.*)\s+(\d+(?::\d+)?)(?:\s+(\d+(?::\d+)?))?\s*$/;
                if (defined $samples2) {
...
foreach (@SortedData) {
        ...
        my ($stack, $samples, $indwall, $samples2, $inddelta) = /^(.*)\s+(\d+)(?::(\d+))?(?:\s+(\d+)(?::(\d+))?)?\s*$/;
