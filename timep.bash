...
#   -r: recursively find source for dependent functions too (requires that your bash binary has the --rpm-requires flag))
    ...
    ${timep_SKIP_DEBUG_FLAG} || {
        ...
        if ((timep_BASH_SUBSHELL_PREV == BASH_SUBSHELL)); then
            ...
            printf '"'"'%s %s %s\n'"'"' "${timep_ENDTIME}" "${timep_END_UTIME}" "${timep_END_STIME}" >>"${timep_TMPDIR}/.log/.endtimes/${timep_NEXEC_0}.${timep_NEXEC_A[-1]}"
        ...
        if ! ${timep_SUBSHELL_INIT_FLAG} && [[ ${timep_BASH_COMMAND_PREV[${timep_FNEST_CUR}]} ]]; then
            ...
            [[ -s "${timep_TMPDIR}/.log/.endtimes/${timep_NEXEC_0}.${timep_NEXEC_A[-1]}" ]] && {
                {
                    while read -r -u ${timep_FD} timep_ENDTIME0 timep_END_UTIME0 timep_END_STIME0; do
                        ((${timep_ENDTIME0//./} < ${timep_ENDTIME//./})) && timep_ENDTIME="${timep_ENDTIME0}"
                        ((${timep_END_UTIME0//./} < ${timep_END_UTIME//./})) && timep_END_UTIME="${timep_END_UTIME0}"
                        ((${timep_END_STIME0//./} < ${timep_END_STIME//./})) && timep_END_STIME="${timep_END_STIME0}"
                    done
        ...
        if [[ "$BASH_COMMAND" == exec* ]]; then
            ...
            if [[ -x "${timep_EXEC_ARG}" ]] && { [[ "${timep_EXEC_ARG}" == "${timep_BASH_PATH}" ]] || [[ "${timep_EXEC_ARG##*/}" == "bash" ]]; }; then
                timep_SKIP_DEBUG_FLAG=true
                ${timep_NO_PRINT_FLAG} || printf '"'"'%s\t%s\t%s\t%s\t%s\tF:%s %s\tS:%s %s\tN:%s %s.%s\t%s\t::\t%s\n'"'"' "${timep_NPIPE[${timep_FNEST_CUR}]}" "${timep_ENDTIME}" "-" "${timep_END_UTIME}" "${timep_END_UTIME}" "${timep_END_STIME}" "${timep_END_STIME}" "${timep_FNEST_CUR}" "${timep_FUNCNAME_STR}" "${BASH_SUBSHELL}" "${timep_BASHPID_STR}" "${timep_NEXEC_N}" "${timep_NEXEC_0}" "${timep_NEXEC_A[-1]}" "${LINENO}" "<< EXEC BASH: ${BASH_COMMAND@Q} >>" >>"${timep_TMPDIR}/.log/log.${timep_NEXEC_0}"
                timep_BASH_COMMAND_PREV[${timep_FNEST_CUR}]="${BASH_COMMAND}"
                timep_FNEST+=("${#FUNCNAME[@]}")
...
## corrects for the overhead of adding nPipe=${#PIPESTATUS[@]} before every command
    ...
    (( timep_RUNTIME_CORRECTION = ( tSum1 - tSum0 + ( NN  >> 1 ) ) / NN ))
...
_timep_PROCESS_LOG() {
    ...
    for (( kk=${#logA[@]}-1; kk>=0; kk-- )); do
        ...
        if [[ "${cmdA[$kk]//"'"/}" == '<< ('*'): '*' >>' ]]; then
            ...
            [[ "${cmdA[$kk]//"'"/}" == '<< (BACKGROUND FORK): '*' >>' ]] || {
                _timep_FILE_EXISTS "${timep_TMPDIR}/.log/.runtimes/log.${nexecA[$kk]#* }" && {
                    read -r runTime uTime sTime <"${timep_TMPDIR}/.log/.runtimes/log.${nexecA[$kk]#* }"
            ...
            [[ "${endTimesA[$kk]}" == '-' ]] && {
                _timep_FILE_EXISTS "${timep_TMPDIR}/.log/.endtimes/log.${nexecA[$kk]#* }" && {
                    read -r endTime endUTime endSTime <"${timep_TMPDIR}/.log/.endtimes/log.${nexecA[$kk]#* }"
                    [[ ${endTime} ]] && ! [[ "${endTime}" == '-' ]] && endTimesA[$kk]="${endTime}"
                    [[ ${endUTime} ]] && ! [[ "${endUTime}" == '-' ]] && endUTimesA[$kk]="${endUTime}"
                    [[ ${endSTime} ]] && ! [[ "${endSTime}" == '-' ]] && endSTimesA[$kk]="${endSTime}"
                }
            }
        else
            ...
            until [[ "${log_tmp}" == *'/log' ]]; do
                [[ -s "${log_tmp}" ]] && {
                    while read -r _ endTime _ ; do
                        (( 10#${endTime//./} > 10#${startTimesA[$kk]//./} )) && break 2
                    done <"${log_tmp}"
                }
                log_tmp="${log_tmp%.*}"
            done
        ...
        if ${inPipeFlag}; then
            ...
            endUTimesA[$kk]="${endUTimesA[$kk1]}"
            endSTimesA[$kk]="${endSTimesA[$kk1]}"
            cmdA[$kk]+=" | ${cmdA[$kk1]// \(\&\)/}"
        ...
        [[ -z ${runTimesA[$kk]} ]] && _timep_EPOCHREALTIME_DIFF "$kk"
        ...
        if (( kk > 0 )) && (( kk < ${#logA[@]} - 1 )) && { { (( uTime == 0 )) && (( uTimesA[$kk1] + uTimeSplitN > 0 )); } || { (( uTimeSplitN > 0 )) && (( nPipeA[$kk] > 1 )); }; }; then
            if (( uTimeSplitN == 0 )) && (( uTime == 0 )) && (( kk > 0 )); then
                uTimeSplitK=${kk1}
                (( wallUTimeSplitA[0] = 10#0${runTimesA[$kk1]//./} / uTimesA[$kk1] ))
                (( wallUTimeSplitA[1] = 10#0${runTime//./} ))
                if (( nPipeA[$kk] == 1 )); then
                ...
                (( wallUTimeSplitA[${#wallUTimeSplitA[@]}] = 10#0${runTime//./} ))
                (( nPipeA[$kk] == 1 )) && {
                    (( wallUTimeSplitSum += wallUTimeSplitA[-1] ))
                    ((uTimeSplitN++))
                }
            fi
        elif (( uTimeSplitN > 0 )) && (( uTimeSplitK > 0 )) && { (( uTime > 0 )) || { (( kk == 0 )) && (( uTimesA[uTimeSplitK] > 0 )); }; }; then
            (( uTimeSplitTimeDistAll = timep_CPU_TIME_MULT - ( timep_RUNTIME_MIN * uTimeSplitN ) ))
            (( wallUTimeSplitAvg2 = ( wallUTimeSplitSum ** 2 ) / ( uTimeSplitN  ** 2 ) ))
            uTimeSplitIWMax=0
        ...
        (( kk1 = kk + 1 ))
        if (( kk > 0 )) && (( kk < ${#logA[@]} - 1 )) && { { (( sTime == 0 )) && (( sTimesA[$kk1] + sTimeSplitN > 0 )); } || { (( sTimeSplitN > 0 )) && (( nPipeA[$kk] > 1 )); }; }; then
            if (( sTimeSplitN == 0 )) && (( sTime == 0 )) && (( kk > 0 )); then
                sTimeSplitK=${kk1}
                (( wallSTimeSplitA[0] = 10#0${runTimesA[$kk1]//./} / sTimesA[$kk1] ))
                (( wallSTimeSplitA[1] = 10#0${runTime//./} ))
                if (( nPipeA[$kk] == 1 )); then
                ...
                (( wallSTimeSplitA[${#wallSTimeSplitA[@]}] = 10#0${runTime//./} ))
                (( nPipeA[$kk] == 1 )) && {
                    (( wallSTimeSplitSum += wallSTimeSplitA[-1] ))
                    ((sTimeSplitN++))
                }
            fi
        elif (( sTimeSplitN > 0 )) && (( sTimeSplitK > 0 )) && { (( sTime > 0 )) || { (( kk == 0 )) && (( sTimesA[sTimeSplitK] > 0 )); }; }; then
            (( sTimeSplitTimeDistAll = timep_CPU_TIME_MULT - ( timep_RUNTIME_MIN * sTimeSplitN ) ))
            (( wallSTimeSplitAvg2 = ( wallSTimeSplitSum ** 2 ) / ( sTimeSplitN  ** 2 ) ))
            sTimeSplitIWMax=0
            ...
            for kk1 in "${!wallSTimeSplitA[@]}"; do
                (( nPipeA[$sTimeSplitK] == 1 )) && (( sTimesA[$sTimeSplitK] = timep_RUNTIME_MIN + ( sTimeSplitTimeDistA[$kk1] * sTimeSplitTimeDistAll / sTimeSplitTimeDistSum ) ))
                (( sTimeSplitK-- ))
                (( sTimeSplitN-- ))
            done
            sTimeSplitK=0
