...
printf '1\t%s\t%s\t-\t-\tF:%s %s\tS:%s %s\tN:%s %s.%s{%s-%s}\t%s\t::\t%s\n' "\${EPOCHREALTIME//./}" "\${timep_END_CTIME}" "\${timep_FNEST_CUR:-\${timep_FUNCNAME_N}}" "\${timep_FUNCNAME_STR}" "\${timep_BASH_SUBSHELL}" "\${timep_BASHPID_STR}" "\${timep_NEXEC_N}" "\${timep_NEXEC_0}" "\${timep_NEXEC_CUR}" "\${timep_NPIDWRAP}" "\${timep_BASHPID}" "\${LINENO}" "\${timep_BASH_COMMAND_PREV_0@Q}" >>"\${timep_TMPDIR}/.log/log.\${timep_NEXEC_HASH_CUR}.init_s"
echo "\${timep_NEXEC_HASH_CUR} --> \${timep_NEXEC_0}" >>"\${timep_TMPDIR}/run.log.txt"
